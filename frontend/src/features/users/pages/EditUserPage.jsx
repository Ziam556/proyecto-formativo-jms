import { useState } from "react";
import { useNavigate, useLocation } from "react-router-dom";
import { ArrowLeft, Camera, User, Search, Plus } from "lucide-react";
import { Input, Select, DatePicker, FileInput, Button} from "@/shared";
import { users } from "../data/users";

// ─── Opciones de selects ──────────────────────────────────────────────────────
const DOC_TYPE_OPTIONS = [
  { value: "CC",  label: "Cédula Ciudadanía" },
  { value: "TI",  label: "Tarjeta de Identidad" },
  { value: "CE",  label: "Cédula de Extranjería" },
  { value: "PPT", label: "PPT" },
  { value: "PEP", label: "PEP" },
];

const USER_TYPE_OPTIONS = [
  { value: "Cliente",        label: "Cliente" },
  { value: "Administrador",  label: "Administrador" },
  { value: "Funcionario",    label: "Funcionario" },
];

// ─── Estado inicial del formulario ───────────────────────────────────────────
const EMPTY_FORM = {
  name:               "",
  documentType:       "",
  document:           "",
  startDate:          "",
  endDate:            "",
  email:              "",
  emailConfirm:       "",
  phone:              "",
  address:            "",
  emailInstitutional: "",
  password:           "",
  userType:           "",
};

function buildForm(u) {
  return {
    name:               u.name               ?? "",
    documentType:       u.documentType        ?? "",
    document:           String(u.document     ?? ""),
    startDate:          u.startDate           ?? "",
    endDate:            u.endDate             ?? "",
    email:              u.email               ?? "",
    emailConfirm:       u.email               ?? "",
    phone:              String(u.phone        ?? ""),
    address:            u.address             ?? "",
    emailInstitutional: u.emailInstitutional  ?? "",
    password:           "",
    userType:           u.userType            ?? "",
  };
}

// ─── Page ─────────────────────────────────────────────────────────────────────
export default function EditUserPage() {
  const navigate    = useNavigate();
  const location    = useLocation();
  const preloaded   = location.state?.user ?? null;

  const [form, setForm]               = useState(preloaded ? buildForm(preloaded) : EMPTY_FORM);
  const [selectedUser, setSelectedUser] = useState(preloaded);
  const [isEnabled, setIsEnabled]     = useState(true);
  const [query, setQuery]             = useState("");
  const [userImage, setUserImage]     = useState([]);
  const [photoPreview, setPhotoPreview] = useState(null);

  const handleImageChange = (files) => {
    setUserImage(files);
    if (files.length > 0) {
      setPhotoPreview(URL.createObjectURL(files[0]));
    } else {
      setPhotoPreview(null);
    }
  };

  const handleChange = (e) => {
    const { name, value } = e.target;
    setForm((prev) => ({ ...prev, [name]: value }));
  };

  // Resultados de búsqueda
  const results = query.trim()
    ? users.filter((u) =>
        u.name?.toLowerCase().includes(query.toLowerCase()) ||
        String(u.document).includes(query) ||
        u.email?.toLowerCase().includes(query.toLowerCase())
      ).slice(0, 10)
    : [];

  const handleSelectUser = (u) => {
    setSelectedUser(u);
    setForm(buildForm(u));
    setQuery("");
  };

  return (
    <div className="min-h-[calc(100vh-72px)] flex flex-col box-border py-5 px-3 sm:px-6 items-center justify-center">

      {/* ── Fila de paneles ── */}
      <div className="w-full max-w-[1280px] flex flex-col lg:flex-row gap-4 items-start">

        {/* ═══════════════ PANEL IZQUIERDO: Avatar + Toggle ═══════════════ */}
        <div className="w-full lg:w-[200px] flex-shrink-0 rounded-2xl bg-[rgba(217,217,217,0.15)] backdrop-blur-[16px] border border-white/20 py-6 px-4 flex flex-col items-center gap-6 relative min-h-[260px]">

          {/* Flecha regresar */}
          <button
            onClick={() => navigate("/dashboard/userpage")}
            className="absolute top-4 left-4 bg-transparent border-0 cursor-pointer text-white hover:text-white/70 transition-colors"
            title="Regresar"
          >
            <ArrowLeft size={22} />
          </button>

          {/* Avatar */}
          <div className="relative mt-6">
            <div className="w-[130px] h-[130px] rounded-full bg-[rgba(200,200,220,0.45)] border-2 border-white/60 overflow-hidden flex items-center justify-center">
              {photoPreview
                ? <img src={photoPreview} className="w-full h-full object-cover" alt="Foto de perfil" />
                : <User size={60} color="rgba(90,90,120,0.7)" />
              }
            </div>
            {/* Ícono cámara */}
            <div className="absolute bottom-1 right-1 w-[32px] h-[32px] rounded-full bg-[#71277A] flex items-center justify-center border-2 border-white pointer-events-none">
              <Camera size={15} color="#fff" />
            </div>
          </div>

          {/* FileInput foto de perfil */}
          <div className="flex flex-col items-center gap-1 w-full">
            <label className="text-white text-[0.72rem] text-center">
              Foto de perfil
            </label>
            <FileInput
              value={userImage}
              onChange={handleImageChange}
              accept="image/*"
              multiple={false}
            />
          </div>

          {/* Botón habilitar / deshabilitar */}
          <button
            type="button"
            onClick={() => setIsEnabled((prev) => !prev)}
            className={`w-full py-3 rounded-lg text-sm font-semibold transition-all duration-300 flex items-center justify-center gap-2 ${
              isEnabled
                ? "bg-emerald-400 hover:bg-emerald-500 text-white shadow-md shadow-emerald-400/40"
                : "bg-gray-400 hover:bg-gray-500 text-white shadow-md shadow-gray-400/40"
            }`}
          >
            {isEnabled ? "✓  Habilitado" : "✕  Deshabilitado"}
          </button>
        </div>

        {/* ═══════════════ PANEL CENTRAL: Formulario ═══════════════ */}
        <div className="flex-1 min-w-0 rounded-2xl bg-[rgba(217,217,217,0.31)] backdrop-blur-[16px] border border-white/20 p-5 sm:p-7">

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">

            {/* Fila 1 */}
            <Input
              label="Nombre"
              name="name"
              placeholder="Name"
              value={form.name}
              onChange={handleChange}
            />
            <Input
              label="Confirmación de correo electrónico"
              name="emailConfirm"
              type="email"
              placeholder="User@email.com"
              value={form.emailConfirm}
              onChange={handleChange}
            />

            {/* Fila 2 */}
            <Select
              label="Tipo de documento"
              name="documentType"
              value={form.documentType}
              options={DOC_TYPE_OPTIONS}
              onChange={handleChange}
              placeholder="Cédula Ciudadanía"
            />
            <Select
              label="Tipo de usuario"
              name="userType"
              value={form.userType}
              options={USER_TYPE_OPTIONS}
              onChange={handleChange}
              placeholder="Tipo de usuario"
            />

            {/* Fila 3 */}
            <Input
              label="Número de documento"
              name="document"
              placeholder="1.999.999.999"
              value={form.document}
              onChange={handleChange}
            />
            {/* Teléfono + botón secundario */}
            <div className="relative">
              <Input
                label="Número telefónico de contacto"
                name="phone"
                type="tel"
                placeholder="300 000 0000"
                value={form.phone}
                onChange={handleChange}
              />
              <button
                type="button"
                title="Agregar número secundario"
                className="absolute right-[10px] bottom-[13px] w-[22px] h-[22px] rounded bg-[rgba(80,80,180,0.25)] border-0 cursor-pointer flex items-center justify-center z-[2] hover:bg-[rgba(80,80,180,0.45)] transition-colors"
              >
                <Plus size={14} color="#fff" />
              </button>
            </div>

            {/* Fila 4 */}
            <DatePicker
              label="Fecha inicio"
              name="startDate"
              placeholder="00/00/0000"
              value={form.startDate}
              onChange={handleChange}
            />
            <Input
              label="Dirección"
              name="address"
              placeholder="Cl 44 #12-51 Pereira"
              value={form.address}
              onChange={handleChange}
            />

            {/* Fila 5 */}
            <DatePicker
              label="Fecha finalización"
              name="endDate"
              placeholder="00/00/0000"
              value={form.endDate}
              onChange={handleChange}
            />
            <Input
              label="Correo institucional (opcional)"
              name="emailInstitutional"
              type="email"
              placeholder="User@soy.sena.edu.co"
              value={form.emailInstitutional}
              onChange={handleChange}
            />

            {/* Fila 6 */}
            <Input
              label="Correo electrónico"
              name="email"
              type="email"
              placeholder="User@email.com"
              value={form.email}
              onChange={handleChange}
            />
            <Input
              label="Contraseña"
              name="password"
              type="password"
              placeholder="••••••••••••"
              value={form.password}
              onChange={handleChange}
            />
          </div>

          {/* Botón Confirmar */}
          <div className="mt-6 flex justify-center">
            <Button
              variant = "primary"
            >
              Confirmar
            </Button>
          </div>
        </div>

        {/* ═══════════════ PANEL DERECHO: Buscador ═══════════════ */}
        <div className="w-full lg:w-[280px] flex-shrink-0 rounded-2xl bg-[rgba(217,217,217,0.15)] backdrop-blur-[16px] border border-white/20 p-4 flex flex-col gap-3">

          {/* Input de búsqueda */}
          <div className="relative">
            <Search
              size={14}
              className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none"
            />
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Buscar por nombre, documento o correo"
              className="w-full bg-white/80 rounded-lg pl-8 pr-3 py-[10px] text-[0.78rem] text-gray-700 outline-none border border-white/30 placeholder:text-gray-400"
            />
          </div>

          {/* Resultados */}
          <div className="bg-white/10 rounded-xl border border-white/20 flex flex-col overflow-hidden">
            <p className="text-white text-xs px-3 py-2 border-b border-white/10 shrink-0 m-0 font-medium">
              Resultados
            </p>
            <div className="flex flex-col overflow-y-auto max-h-[340px]">
              {!query.trim() && (
                <p className="text-white/40 text-[0.73rem] text-center py-6 px-3">
                  Busca un usuario para editar
                </p>
              )}
              {query.trim() && results.length === 0 && (
                <p className="text-white/50 text-[0.73rem] text-center py-6">
                  Sin resultados
                </p>
              )}
              {results.map((u) => (
                <button
                  key={u.id}
                  type="button"
                  onClick={() => handleSelectUser(u)}
                  className={`flex items-center gap-2 px-3 py-[10px] text-left hover:bg-white/20 transition-colors border-b border-white/10 last:border-0 w-full ${
                    selectedUser?.id === u.id ? "bg-white/25" : ""
                  }`}
                >
                  <div className="w-8 h-8 rounded-full bg-[rgba(200,200,220,0.45)] flex items-center justify-center flex-shrink-0 border border-white/30">
                    <User size={15} color="rgba(90,90,120,0.9)" />
                  </div>
                  <div className="flex flex-col min-w-0">
                    <span className="text-white text-[0.75rem] font-semibold truncate leading-tight">
                      {u.name}
                    </span>
                    <span className="text-white/55 text-[0.68rem] truncate leading-tight">
                      {u.documentType} · {u.document} · {u.email}
                    </span>
                  </div>
                </button>
              ))}
            </div>
          </div>
        </div>

      </div>
    </div>
  );
}
